...
if not GROQ_API_KEY:
    try:
        GROQ_API_KEY = st.secrets.get("GROQ_API_KEY", None)   # ← preferred way in Streamlit Cloud
    except Exception:
        GROQ_API_KEY = None

if not GROQ_API_KEY:
    st.error("GROQ_API_KEY not found. Please set it in your .env file, secrets, or environment.")
    st.stop()
...
st.markdown("Upload your resume (PDF) and paste the job description to get a detailed match analysis powered by Groq (Llama 3.3).")
...
if st.button("Analyze Resume Match", type="primary", disabled=not (uploaded_file and job_description.strip())):
    ...
    with st.spinner("Analyzing with Groq (this can take 10–30 seconds)..."):
